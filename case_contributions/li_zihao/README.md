...
This folder contains five additional Problem A evaluation cases. They follow
the shared `EXTRA_*` contribution format and do not modify any supplied row.
...
- `cases.py` contains the five new claim records.
- `labels.json` contains labels fixed before agent evaluation.
- `README.md` records ownership and the intended decision boundaries.

## Integration

Merge the `EXTRA_*` values into `reference_data/make_fixtures_A.py` and merge
the five labels into `reference_data/expected_outcomes_A.json`. Then regenerate
and validate the shared dataset:
